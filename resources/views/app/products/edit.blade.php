                    ...
                    <form action="{{ route('products.update', $product->slug) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="mb-4 d-flex flex-column gap-3 align-items-center justify-content-center">
                            <div id="image-preview" class="user-change-photo shadow">
                                <img src="{{ url($product->thumbnail) }}" alt="Thumbnail">
                            </div>
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-4">
                                ...
                                <input type="number" class="form-control @error('stock') is-invalid @enderror" name="stock" value="1" value="{{ old('stock', $product->stock) }}">
                                @error('stock')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            <div class="col-12">
                                ...
                                <select class="form-select text-capitalize @error('category') is-invalid @enderror" name="category">
                                    ...
                                    @forelse ($categories as $category)
                                        <option value="{{ $category->slug }}" {{ old('categori', $product->category->slug) == $category->slug ? 'selected' : '' }} class="text-capitalize">{{ $category->name }}</option>
                                    @empty
                            ...
                            <div class="col-12">
                                ...
                                <textarea class="form-control @error('description') is-invalid @enderror" name="description" rows="4" cols="4" placeholder="Describe product...">{{ old('description', $product->description) }}</textarea>
                                @error('description')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        <div class="row g-3 my-3" id="galleries-preview">
                            @forelse($product->file_products as $gallery)
                                <div class="col-md-4 shadow">
                                    <img src="{{ url($gallery->image) }}" alt="logo" class="img-thumbnail">
                                </div>
                            @empty
                                <div class="col-md-12 shadow">There's no any gallery!</div>
                            @endforelse
                        </div>
